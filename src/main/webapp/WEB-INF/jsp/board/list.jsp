...
<script>
	function goWriteForm(){
		location.href = "writeForm.do";  <%-- ${pageContext.request.contextPath}/board/ --%>
	}
		...
		<%-- <c:choose>
			...
			<c:otherwise>
				if(confirm('로그인이 필요합니다. 로그인하시겠습니까?')){
					location.href = "${pageContext.request.contextPath}/member/loginForm.do?url=board/detail&no="+boardNo;
				}
			</c:otherwise>
		</c:choose> --%>
...
<body>
	...
	<section>
		...
		<%-- <%=listAll.size() %> --%>
		<!-- begin="0" end="5" -->
		<c:forEach var="i" items="${list}" step="${size}" varStatus="loop">
